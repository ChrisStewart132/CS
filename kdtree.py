...
class KdTree:
    ...
    def points_in_range(self, rectangle):
        ...
        if self.is_leaf:
            ...
            matches = []
            try:
                if self.leftorbottom != None and bl_point[self.axis] <= self.coord:
                    matches += self.leftorbottom.points_in_range(rectangle)
            except AttributeError:
                pass
            try:
                if self.rightortop != None and tr_point[self.axis] >= self.coord:
                    matches += self.rightortop.points_in_range(rectangle)
            except AttributeError:
                pass
        return matches
...
class QuadTree:
    ...
    def __init__(self, points, centre, size, depth=0, max_leaf_points=2):
        ...
        #print(bottom_left,top_right,self.points)     
        if len(self.points) > max_leaf_points and self.depth < self.MAX_DEPTH:
            ...
            self.is_leaf = True
            #self.points = points
        #return QuadTree(points, centre, size, children)
